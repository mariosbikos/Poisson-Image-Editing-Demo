...
%that don't belong to the mask, we also add(for creating b vector)
%the laplacian value of each pixel of the mask calculated from the source.

clear all;
close all;

%Read the input images
srcImg    = rgb2gray(imread('tatoo.jpg'));
targetImg = rgb2gray(imread('me2.jpg'));

%Reshape the target image from 2D to 1D
targetImg_1D=reshape(targetImg,size(targetImg,1)*size(targetImg,2),1);
srcImg_1D=reshape(srcImg,size(srcImg,1)*size(srcImg,2),1);
...
stepsA = size(Mask_Source_1D_Indices,1)*size(Mask_Source_1D_Indices,1);
stepA=1;
...
%Show the re-mapped mask of source to target in a new image
figure(fig_src_trg);
subplot(2,2,4);
imshow(MaskOfTarget);
...
b=zeros(size(Mask_Source_1D_Indices,1),1);
...
for i=1:size(Mask_Source_1D_Indices,1)
    ...
    b(i)=...
        SumOfNeighbors( MaskOfTarget_indices(i),targetImg_1D,MaskOfTarget_1D,height_MaskOfTarget )+...
        SumOfLaplacians( Mask_Source_1D_Indices(i),srcImg_1D,height_MaskOfSource );
end
close(Bwait);
...
imwrite(newTargetImg,'Result_3a_Image.png')
print(res_fig,'Result_3a_Figure','-dpng')
